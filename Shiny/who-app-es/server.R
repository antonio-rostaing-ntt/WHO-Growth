...
shinyServer(function(input, output) {
  ...
  output$age <- renderText({ 
    paste(data.days(), " días - ", round(data.days()/7,0), ".", data.days()%%7, "semanas")
  })
  ...
  output$weightPlot <- renderChart({
    ...
    h1$title(text = 'Percentiles de peso', x = -20)
    h1$subtitle(text = paste(year+1, ' año/s'), x = -20)
    h1$xAxis(title = list(text = 'Días'), labels=list(overflow='justify'), categories = plotData$age, tickInterval=31)
    h1$yAxis(title = list(text = 'Peso (kg)', plotlines = list (value = 0, width = 1, color = '#808080')))
  ...
  output$heightPlot <- renderChart({
    ...
    h1 <- Highcharts$new()
    h1$chart(type = "spline")
    h1$title(text = 'Percentiles de estatura', x = -20)
    h1$subtitle(text = paste(year+1, ' año/s'), x = -20)
    h1$xAxis(title = list(text = 'Días'), labels=list(overflow='justify'), categories = plotData$age, tickInterval=31)
    h1$yAxis(title = list(text = 'Estatura (cm)', plotlines = list (value = 0, width = 1, color = '#808080')))
